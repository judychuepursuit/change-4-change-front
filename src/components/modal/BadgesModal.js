...
export default function RewardsModal({ isModalOpen, setIsModalOpen }) {
  ...
  useEffect(() => {
    ...
    console.log(
      `Twitter Link: http://twitter.com/share?url=${link}&text=${msg}&hashtags=donate,charity,support,kindness`
    );

    const c4cLink = "https://change-4-change-frontend.onrender.com/";
    const fb = document.querySelector(".facebook");
    // fb.href = `https://www.facebook.com/share.php?u=${link}`;
    fb.href = `https://www.facebook.com/dialog/share?app_id=868771211620111&display=popup&href=${encodeURIComponent(
      c4cLink
    )}`;
    ...
    twit.href = `http://twitter.com/share?url=${encodeURIComponent(
      c4cLink
    )}&text=${msg}&hashtags=donate,charity,support,kindness`;
  }, []);
  ...
  return (
    <div
      onClick={closeModal}
      className="overlay"
      style={{ display: isModalOpen ? "fixed" : "none" }}
    >
      {isModalOpen && (
